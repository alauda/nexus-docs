...
**If you have already upgraded to version 3.81, please follow the [Rollback to Nexus 3.76](./04_rollback_to_3.76.mdx) guide to downgrade.**
...
Backup consists of five steps:

1. Uninstall the Nexus operator from the cluster if not done already.
2. Create a backup task on Nexus web.
3. Remove the nexus instance service to prevent data changes
4. Backup nexus data and verify the backup.
5. Restore nexus instance after the backup end.
...
When restoring data, ensure you use the correct database file. 

If rolling back to Nexus 3.76, use the rolled-back database file created in the previous step of the rollback process. See [Rollback to Nexus 3.76](./04_rollback_to_3.76.mdx#rollback-database) for details.
